...
export async function getReport(reportId: string): Promise<Report | null> {
  ...
  // Mirror the enrichment from getReports so the detail page can show
  // human names instead of raw UIDs.
  let reporterName: string | undefined;
  let reportedUserName: string | undefined;
  try {
    const [reporterDoc, reportedUserDoc] = await Promise.all([
      data.reporterId
        ? getDoc(doc(db, COLLECTIONS.USERS, data.reporterId))
        : Promise.resolve(null),
      data.reportedUserId
        ? getDoc(doc(db, COLLECTIONS.USERS, data.reportedUserId))
        : Promise.resolve(null),
    ]);
    if (reporterDoc?.exists()) reporterName = reporterDoc.data().name;
    if (reportedUserDoc?.exists()) reportedUserName = reportedUserDoc.data().name;
  } catch (e) {
    console.error('Error fetching report user names:', e);
  }
...
export async function getUsers(searchTerm?: string): Promise<User[]> {
  // No server-side orderBy — Firestore silently drops docs missing the field,
  // which excludes early users (including admins) that pre-date `createdAt`.
  // Sort client-side instead so every user shows up.
  ...
  // Newest first; users without `createdAt` fall to the bottom.
  users.sort((a, b) => {
